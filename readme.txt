...
The complete uncompiled source code is public at https://github.com/imrantushar/gameengine and also ships inside the plugin, in `src/` and `assets/scss/`. See "Source code" below.
...
The only generated files are in `assets/build/`. Each is named after the source file it is built from, and each carries a comment at the top of the file naming that source and the command that rebuilds it:

*   `assets/build/backend.js`  — built from `src/backend.js` (the admin app)
*   `assets/build/frontend.js` — built from `src/frontend.js` (the frontend script)
*   `assets/build/setup.js`    — built from `src/setup.js` (the setup wizard)
...
Those three entry points pull in the rest of `src/`, the complete uncompiled React source: `components/`, `containers/`, `redux/`, `hooks/`, `utils/` and `frontend/`. Nothing in this plugin is obfuscated, and nothing is minified without its source beside it.
...
= 1.3.3 - 2026-09-07 =
* Changed - The uncompiled React source moved from `dev_gameengine/` to `src/`, so each generated file in `assets/build/` sits beside a source directory named by the usual convention. No code changed with it.
* Added - Every generated file in `assets/build/` now begins with a comment naming the source file it was built from, where the rest of the source is, and the command that rebuilds it.
...
= 1.3.3 =
Documentation and source layout only. No functional change.
