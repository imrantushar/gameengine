...
export default function Wallet() {
  ...
  const fetchPayouts = async ({
    ...
    try {
      await dispatch(fetchPayoutList({
        status,
        page,
        per_page,
        search
  ...
  const columns = [{
    ...
    cell: row => <p className="font-medium">{row.display_name || '—'}</p>,
    columnWidth: "22%"
  }, {
    ...
    cell: row => {
      ...
      const visibleText = showFull || !isLong ? notes : notes.substring(0, maxVisible).trim() + '...';
      return <div className="flex flex-col gap-1">
          ...
          "color": notes === '—' ? 'gray.500' : 'inherit'
                        ...
                        {isLong && <button className="p-0 h-auto self-center" style={{
          "color": "blue"
    ...
    cell: row => {
      ...
      return <StatusOptions value={row?.status || 'pending'} options={{
  ...
  return <>
        ...
        "padding": "24px 0"
                ...
                <ListTable key={'wallet-payouts-' + payouts.length} columns={columns} showColumnFilter={false} data={payouts} showSubHeader={true} subHeaderComponent={subHeaderComponentMemo} isRowSelectable={false} showPagination={false} noDataText={__('No data found for Wallet', 'gameengine')} totalItems={total} totalRows={total} rowsPerPage={perPage || 10} currentPageNumber={[1]} dataFetchingStatus={dataFetchingStatus} getSelectRowValue={setSelectedRows} />
            </div>
        </>;
}
;