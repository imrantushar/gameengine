...
const FormInner = () => {
  ...
  const renderHookCard = (item, type) => {
    ...
    return (
      <DraggableItem key={`${type}_${item.id}`} id={`${type}_${item.id}`} onActivate={() => handleDragEnd({ active: { id: `${type}_${item.id}` }, over: { id: 'awards-sidebar' } })}>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="gameengine-add-achievement-type flex gap-3">
        ...
        >
          <Select
            ...
            value={achievementTypes?.find(opt => Number(opt.value) === Number(values?.category_id)) || null}
        ...
        <div className="flex gap-3">
          <GameEngineInput label={__("Required Achievements", "gameengine")} width="calc(50% - 6px)">
            <Select
              ...
              value={achievementsData?.find(opt => Number(opt.value) === Number(values?.required_achievement_id)) || null}
          ...
          <GameEngineInput
            label={__("Required Levels", "gameengine")}
            width="calc(50% - 6px)"
          >
            <Select
              className="gameengine-select"
      ...
      {values?.unlock_with_points_enabled ? (
        <div className="gameengine-allow-unlock-point flex gap-3">
          <GameEngineInput
            label={__("Points", "gameengine")}
            width="calc(50% - 6px)"
          >
            <input
              className="gameengine-input"
              placeholder={__("Enter point", "gameengine")}
              type="number" value={values.required_points_amount}
              onChange={e => {
          ...
          <GameEngineInput
            label={__("Choose the Points Type", "gameengine")}
            width="calc(50% - 6px)"
          >
            <Select
              className="gameengine-select"
