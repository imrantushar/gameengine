  ...
  return <SettingsInner heading={__(`Email Notification`, "gameengine")}>
            <div className="flex flex-col gap-4">
                ...
                <GameEngineInput label={__("From Name", "gameengine")}>
                    <input placeholder={__("Enter from name", "gameengine")} value={email?.from_name || ''} onChange={e => setFieldValue('email.from_name', e.target.value)} style={commonInput} />
                </GameEngineInput>

                <GameEngineInput label={__("From Address", "gameengine")}>
                    <input placeholder={__("Enter from address", "gameengine")} value={email?.from_address || ''} onChange={e => setFieldValue('email.from_address', e.target.value)} style={commonInput} />
                </GameEngineInput>
