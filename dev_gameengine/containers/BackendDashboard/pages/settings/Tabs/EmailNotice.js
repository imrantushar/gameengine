      ...
      <div className="flex flex-col gap-4">
        <GameEngineInput label={__('Format', 'gameengine')}>
          <Select
            ...
            value={formatOptions.find(
              (opt) => opt?.value === email?.format
            )}
            onChange={(opt) =>
              setFieldValue('email.format', opt.value)
            }
          />
        </GameEngineInput>
        ...
        <GameEngineInput label={__('Schedule', 'gameengine')}>
          <Select
            ...
            value={scheduleOptions.find(
              (opt) => opt?.value === email?.schedule
            )}
            onChange={(opt) =>
              setFieldValue('email.schedule', opt.value)
            }
          />
        </GameEngineInput>
        ...
        <GameEngineInput label={__('From Name', 'gameengine')}>
          <input
            ...
            onChange={(e) =>
              setFieldValue('email.from_name', e.target.value)
            }
          />
        </GameEngineInput>
        ...
        <GameEngineInput label={__('From Address', 'gameengine')}>
          <input
            ...
            onChange={(e) =>
              setFieldValue('email.from_address', e.target.value)
            }
          />
        </GameEngineInput>

        <GameEngineInput label={__(
        ...
        >
          {__(
            'Available placeholders: {user_name}, {level_name}, {site_name}',
            'gameengine'
          )}
        </p>
      </div>
