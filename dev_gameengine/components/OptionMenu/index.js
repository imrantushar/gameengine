...
const OptionMenu = (props) => {
	...
	const renderOptions = () => (
		<div
			className={`gameengine-dropdown-menu__lists  ${alwaysShowOptions ? 'gameengine-dropdown-menu--inline' : ''
				} ${suffix && `gameengine-dropdown-menu--list-${suffix}`}`}
			ref={menuItemRef}
		>
			<ul
				className={`${alwaysShowOptions
						? 'gameengine-dropdown-menu__inline'
						: 'gameengine-more-options'
					}`}
			>
					...
					return (
						<React.Fragment key={itemIndex}>
							{item.action ? (
								<form
									className={`${alwaysShowOptions
											? 'gameengine-dropdown-menu__inline-form'
											: 'gameengine-more-options__item'
										}`}
									action={item.action}
									method={item.method}
								>
									...
									/>
									{item.hasBorder && (
										<hr className="gameengine-option-separator" />
									)}
								</form>
							) : (
								<li
	...
	return (
		<>
			{!alwaysShowOptions && (
				<button
					className={`gameengine-dropdown-menu ${suffix && `gameengine-dropdown-menu--${suffix}`
						}`}
					type="button"
					ref={relativeTo}
