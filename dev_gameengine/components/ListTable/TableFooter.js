...
const options = [{
  value: '10',
  label: __('10', 'gameengine')
}, {
  value: '15',
  label: __('15', 'gameengine')
}, {
  value: '20',
  label: __('20', 'gameengine')
}, {
  value: '25',
  label: __('25', 'gameengine')
}, {
  value: '30',
  label: __('30', 'gameengine')
}];
const TableFooter = ({
  data,
  totalItems,
  ...
  return <div className="gameengine-table__footer">
			<p className="gameengine-table__footer-label">
				{/* eslint-disable-next-line */}
				{sprintf(
      // translators: %1$s: Item showing, %2$s: totalItems
      __('Showing result %1$s out of %2$s', 'gameengine'), data?.length, totalItems)}
			</p>
			...
			<div className="gameengine-table__footer-right">
				<div className="gameengine-table__footer-pagination-per-page">
					...
					<Select id='gameengine-table-row-per-page-selector' menuPlacement='top' className='gameengine-select gameengine-select--65' classNamePrefix='gameengine-select' options={options} value={rowsPerPage ? options.find(item => Number(item.value) === Number(rowsPerPage)) : options[0]} onChange={paginationPerPageChange} />
				</div>

				<div className="gameengine-table__footer-pagination-pages icons">
					<Pagination totalItems={totalItems} fetchHandler={onChangePage} currentPageNumber={currentPageNumber} rowsPerPage={rowsPerPage} />
				</div>
			</div>
		</div>;
};
