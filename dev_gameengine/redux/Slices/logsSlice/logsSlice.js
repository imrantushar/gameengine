...
// --- 1. Fetch Logs ---
export const fetchLogs = createAsyncThunk('gameengine/fetchLogs',
...
export const createLogAction = createAsyncThunk('gameengine/createLogAction',
    async (payload, thunkAPI) => {
        try {
            const response =  await API.post(namespace + 'actions/manual', {...payload});
            thunkAPI.dispatch(showNotification({
                message: __('Log created successfully!', 'gameengine'),
                isShow: true,
                type: 'success',
            }))
...
export const updateLogAction = createAsyncThunk('gameengine/updateLogAction',
    async (payload , thunkAPI) => {
        try {
            const response =  await API.post(namespace + 'logs/' + payload.id, {...payload});
            thunkAPI.dispatch(showNotification({
                message: __('Log updated successfully!', 'gameengine'),
                isShow: true,
                type: 'success',
            }))
            return payload;
