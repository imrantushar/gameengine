...
const Notification = () => {
	...
	const targetElement = document.querySelector('#gamify-admin-app');
	...
	return (
		<>
			{notification?.isShow &&
				createPortal(
					<div
						className={`gamify-notification ${notification.type &&
							`gamify-notification--${notification.type}`
							}`}
						ref={notificationRef}
					...
					>
						{notification?.linkTo ? (
							<Link to={notification.linkTo}>
								<Box textDecoration={'none'} className="gamify-notification__message">
							...
							<div className="gamify-notification__message">
								<Icon as={iconType(notification?.type)} size="md" />
