...
import { CloseButton, Button, Text, Spinner } from '@chakra-ui/react';
...
const ReactModalFormik = ({
	...
	return (
		<div
			ref={overlayRef}
			className={`gamify-modal-overlay gamify-modal-overlay--${position}`}
		...
		>
			...
			>
				<Formik {...formik}>
					{({ isSubmitting }) => (
						<Form>
							<div className="gamify-modal__head">
								{title && <Text fontSize="18px" fontWeight="600" lineHeight="20px" color="var(--gamify-font-black)">{title}</Text>}
								<div className="gamify-modal-buttons">
									{isEnabledResize && (
										<OptionMenu
											...
											options={[
												...
												{
													type: 'button',
													label: <span>{__('Medium screen', 'gamify')}</span>,
													icon: MdLaptopMac,
									...
									<CloseButton onClick={onRequestClose} disabled={isSubmitting} 
                    {...xCloseBtn} 
                  />
								</div>
							</div>
							...
							{isEnabledFooter && (
								<div className="gamify-modal__footer">
									...
									<Button type="submit" loading={isSubmitting} {...primaryBtn} disabled={isSubmitting}>
										{isSubmitting ? (
											<>
												<Spinner
													color="var(--gamify-primary-color)"
													css={{ "--spinner-track-color": "var(--gamify-secondary-color)" }}
												/>
